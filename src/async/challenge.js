...
const anotherFunction = async (urlApi) => {
    ...
    try{
        const products = await fetchData(`${urlApi}/products`);// todos los lelementos 
        const product = await fetchData(`${urlApi}/products/${products[0].id}`); // solo un elemento 
        const category = await fetchData(`${urlApi}/categories/${product.category.id}`); //categoria en particular por id
